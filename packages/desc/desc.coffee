...
class DescFitting
	...
	getShipAttributes: (attrIDs) ->
		...
		i = attrIDs.length + 1
		while i--
 			attr[attrIDs[i]] = @dogmaContext.getShipAttribute(attrIDs[i])
  		attr
				...
				if typeHasEffect(m.module, m.state, e)
					...
					switch e
						when @EFFECT_MISSILES
							...
							emDamage = @dogmaContext.getCharacterAttribute(
								m.key, @ATTR_EMDAMAGE)
							explosiveDamage = @dogmaContext.getCharacterAttribute(
								m.key, @ATTR_EXPLOSIVEDAMAGE)
							kineticDamage = @dogmaContext.getCharacterAttribute(
								m.key, @ATTR_KINETICDAMAGE)
							thermalDamage = @dogmaContext.getCharacterAttribute(
								m.key, @ATTR_THERMALDAMAGE)
							
							dps = (multiplier * (emDamage + explosiveDamage + kineticDamage + thermalDamage)) / effectAttributes.duration

							unless result.missile?
...
Desc.ParseEFT = (fitting) ->
	...
	[parse.loadout.low, parse.loadout.mids, parse.loadout.highs, parse.loadout.rigs, parse.loadout.subs] = racks
